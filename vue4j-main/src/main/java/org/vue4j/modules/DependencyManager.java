...
public class DependencyManager {
     ...
     */
    private final Set<String> buildinDependencies = new HashSet<>();

    private final Map<String, Set<String>> dependenciesByModule = new HashMap<>();
    ...
    private List<URL> loadDependencies(File pom, boolean downloadWithMaven)
        ...
        if (!dependenciesByModule.containsKey(model.getArtifactId())) {
            dependenciesByModule.put(model.getArtifactId(), new HashSet());
        }

        LOGGER.debug(String.format("Maven model resolved: %s, parsing its dependencies...", model));

        // For each dependencies
        Set<String> dependenciesSet = dependenciesByModule.get(model.getArtifactId());
        for (org.apache.maven.model.Dependency d : model.getDependencies()) {
            if (!d.getType().equals("war")) {
                
                dependenciesSet.add(d.getArtifactId());
    ...
    public Set<String> getModuleDependencies(String moduleId) {
        return dependenciesByModule.get(moduleId);
    }
